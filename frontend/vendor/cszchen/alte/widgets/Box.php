...
class Box extends Widget
{
    ...
    public $footer = '';
    /**
     * it must be set when refresh tool be used;
     * @var string
     */
    public $refreshUrl = '';
    ...
    protected $sysTools = [self::TOOL_COLLAPSE, self::TOOL_REFRESH, self::TOOL_REMOVE];
    ...
    protected function refresh()
    {
        ...
        $icon = Html::tag("i", "", ["class" => "fa fa-repeat"]);
        $id = $this->options['id'];
        $this->getView()->registerJs("$('#$id').boxRefresh({source:'{$this->refreshUrl}'})");
        return Html::tag("button", $icon, ["class" => "btn btn-box-tool refresh-btn", "id" => $id, "data-widget" => "refresh"]);
    }
}
